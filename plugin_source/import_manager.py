...
def import_webresult(webresult, input_hash):    
    ...
    for subscription in webresult:     
        if input_hash: # New deck
            ...
            strings_data = mw.addonManager.getConfig(__name__)
            for hash, details in strings_data.items():
                if hash == input_hash and details["deckId"] == 0: # should only be the case once when they add a new subscription and never ambiguous
...
def handle_pull(input_hash):
    ...
    if strings_data is not None and len(strings_data) > 0:
        if "settings" in strings_data:
            ...
            strings_data_to_send = strings_data_copy if input_hash is None else {input_hash: strings_data_copy[input_hash]}
        else:
            strings_data_to_send = strings_data if input_hash is None else {input_hash: strings_data[input_hash]}
        
        response = requests.post("https://plugin.ankicollab.com/pullChanges", json=strings_data_to_send)
